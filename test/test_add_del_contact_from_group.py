...
def test_delete_contact_from_group(app, db, check_ui):
    ...
    old_groups = db.get_group_list()
    selection_group = random.choice(old_groups)
    app.contact.view_contact_from_group(selection_group.id)
    if len(app.contact.get_contacts_list()) == 0:
        app.contact.create(contact)
    group_contact_list = app.contact.get_contacts_list()
    contact = random.choice(group_contact_list)
    app.contact.delete_contact_from_group(contact.id)
    if check_ui:
        assert sorted(db.get_contact_list(), key=Contact.id_or_max) == sorted(app.contact.get_contacts_list(), key=Contact.id_or_max)
